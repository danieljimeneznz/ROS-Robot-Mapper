...
void laserScanCallback(const sensor_msgs::LaserScan::ConstPtr& laserScanData) {
  ...
  int rangeDataNum = int(1 + (laserScanData->angle_max - laserScanData->angle_min)  / (laserScanData->angle_increment));
...
int main (int argc, char **argv) {	
  ros::init(argc, argv, "pioneer_robot_node");	// command line ROS arguments
  ros::NodeHandle handle;	// ROS comms access point
...
//  ros::Publisher vel_pub_object = my_handle.advertise<geometry_msgs::Twist>("/RosAria/cmd_vel",1);
  ...
  */
  ros::Subscriber laser_sub_object = handle.subscribe("/scan", 1, laserScanCallback);
